...
async function films_laadFilms(reset = false) {
    ...
    const params = new URLSearchParams({
        pagina: filmsState.pagina, per_pagina: filmsState.perPagina, sort: filmsState.sort
    });
    ...
    try {
        ...
        if (meerBtn) meerBtn.style.display = (filmsState.pagina * filmsState.perPagina) >= data.totaal ? 'none' : 'block';

    } catch (err) {
        console.error('Films laden mislukt:', err);
        films_verwijderSkeletons();
        document.getElementById('films-grid').innerHTML = `<div class="films-leeg"><p>⚠ Kon films niet laden: ${err.message}</p></div>`;
    }
...
function films_renderFilms(filmLijst) {
    ...
    if (!filmLijst || filmLijst.length === 0) {
        grid.innerHTML = `<div class="films-leeg"><div class="films-leeg__icoon">🎬</div><p>Geen films gevonden.</p></div>`;
        return;
    }
    filmLijst.forEach(film => grid.appendChild(films_maakKaart(film)));
}
...
function films_toonSkeletons() {
    ...
    for (let i = 0; i < 8; i++) {
        ...
        sk.innerHTML = `<div class="films-skeleton__poster"></div>
...
function films_updateTeller(totaal) {
    ...
    if (el) el.textContent = `${totaal} film${totaal !== 1 ? 's' : ''} gevonden`;
}

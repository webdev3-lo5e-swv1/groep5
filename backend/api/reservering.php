...
if ($methode === 'GET') {
    ...
    $voorstellingId = (int) ($body['voorstelling_id'] ?? 0);
    $stoelIds       = $body['stoel_ids'] ?? [];

    if (!$voorstellingId || empty($stoelIds)) {
        http_response_code(400);
        echo json_encode(['success' => false, 'message' => 'Voorstelling of stoelen ontbreken']);
        exit;
    }
    ...
    try {
        ...
        if (!$row) {
            ...
            echo json_encode(['success' => false, 'message' => 'Voorstelling niet gevonden']);
            exit;
        }
    ...
    if ($code) {
        echo json_encode(['success' => true, 'code' => $code, 'totaal' => $totaal]);
    } else {
        http_response_code(500);
        echo json_encode(['success' => false, 'message' => 'Reservering mislukt']);
    }
}